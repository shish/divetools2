...
const STATIONS = [
    ...
    {
        name: "Distress, Urgency, Safety & Calling",
        channels: [16],
        standard: true,
    },
    ...
    // Ports
    {
        name: "Dublin Port",
        channels: [12],
        coords: { lat: 53.3494, lon: -6.2022 },
    },
    {
        name: "Dun Laoghaire Port",
        channels: [14],
        coords: { lat: 53.3024, lon: -6.1264 },
    },

    // Coast guard: http://www.malinheadcoastguardradio.com/vhf%20network.html
    // Dublin MRCC
    {
        name: "Carlingford Coast Guard",
        channels: [67, 4],
        coords: { lat: 54.04, lon: -6.16 },
    }, // approx
    {
        name: "Dublin Coast Guard",
        channels: [67, 83],
        coords: { lat: 53.33, lon: -6.17 },
    }, // approx
    {
        name: "Wicklow Head Coast Guard",
        channels: [67, 2],
        coords: { lat: 52.98, lon: -6.04 },
    }, // approx
    {
        name: "Rosslare Coast Guard",
        channels: [67, 23],
        coords: { lat: 52.27, lon: -6.38 },
    }, // approx
    {
        name: "Minehead Coast Guard",
        channels: [67, 83],
        coords: { lat: 51.19, lon: -3.46 },
    }, // approx
    // Valentia MRSC
    {
        name: "Cork Coast Guard",
        channels: [67, 26],
        coords: { lat: 51.8, lon: -8.27 },
    }, // approx
    {
        name: "Mizen Head Coast Guard",
        channels: [67, 4],
        coords: { lat: 51.45, lon: -9.81 },
    }, // approx
    {
        name: "Bantry Coast Guard",
        channels: [67, 23],
        coords: { lat: 51.68, lon: -9.46 },
    }, // approx
    {
        name: "Valentia Coast Guard",
        channels: [67, 24],
        coords: { lat: 51.9, lon: -10.36 },
    }, // approx
    {
        name: "Shannon Coast Guard",
        channels: [67, 28],
        coords: { lat: 52.45, lon: -9.75 },
    }, // approx
    {
        name: "Galway Coast Guard",
        channels: [67, 4],
        coords: { lat: 53.25, lon: -9.0 },
    }, // approx
    // L. Derg? 16, 61
    // Malin Head MRSC
    // L. Ree? 16, 62
    {
        name: "Clifden Coast Guard",
        channels: [67, 26],
        coords: { lat: 53.48, lon: -10.02 },
    }, // approx
    {
        name: "Belmullet Coast Guard",
        channels: [67, 83],
        coords: { lat: 54.27, lon: -9.99 },
    }, // approx
    {
        name: "Donegal Bay Coast Guard",
        channels: [67, 2],
        coords: { lat: 54.36, lon: -8.67 },
    }, // approx
    {
        name: "Glen Head Coast Guard",
        channels: [67, 24],
        coords: { lat: 54.64, lon: -8.73 },
    }, // approx
    {
        name: "Malin Head Coast Guard",
        channels: [67, 23, 85],
        coords: { lat: 55.38, lon: -7.37 },
    }, // approx
];
...
const StationList = ({ state, stations }) =>
    stations.map((station: Station) => (
        <tr>
            <td>
                {station.channels.map((ch: number) =>
                    DUPLEX.includes(ch) ? (
                        <span class={"duplex channel"}>{ch}</span>
                    ) : (
                        <span class={"simplex channel"}>{ch}</span>
                    ),
                )}
            </td>
            <td>-</td>
...
const EnableGeoButton = () => (
    <a class={"button"} onclick={EnableGeo}>
        Search GPS
    </a>
);
...
const GeoSearch = ({ state }: { state: State }) => [
    ...
    state.location ? (
        ...
        <tr>
            <th colspan={3}>(Can't detect GPS location)</th>
        </tr>
    ),
];
...
export const VhfChannels = (state: State) => (
    <Screen
        ...
        footer={[
            <BackButton />,
            navigator.geolocation && !state.settings.geo_enabled && (
                <EnableGeoButton />
            ),
        ]}
    >
        ...
        <table class={"vhf"}>
            {state.vhf_channels.search ? (
                    ...
                    stations={STATIONS.filter(
                        (x) =>
                            x.name
                                .toLowerCase()
                                .indexOf(
                                    state.vhf_channels.search.toLowerCase(),
                                ) !== -1,
                    )}
                />
            ) : (
                <tbody>
                    <tr>
                        <th colspan={3}>Standard Channels:</th>
                    </tr>
                    <StationList
                        state={state}
                        stations={STATIONS.filter((x) => x.standard)}
                    />
